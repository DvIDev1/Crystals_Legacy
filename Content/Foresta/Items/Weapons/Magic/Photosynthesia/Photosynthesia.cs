    ...
    public class Photosynthesia : ModItem
    {
            ...
            int damage, float knockback)
        {
...

            {
                ...
                Projectile proj = Projectile.NewProjectileDirect(Item.GetSource_None(), position, perturbedSpeed, type, damage, knockback,
                    player.whoAmI);
                proj.CritChance = Item.crit;
                projectiles.Add(proj); //create the projectile
            }
            return false;
        }
        ...
        class MagicLeaves : ModProjectile
        {
            ...
            public override void AI()
            {
                ...
                else if(deadleaf != true)
                {
                    ...
                    Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver2;
                    Projectile.velocity.X += Projectile.DirectionTo(owner.Center).X;
                    Projectile.velocity.X = MathHelper.Clamp(Projectile.velocity.X, -15f, 15f);
                    Projectile.velocity.Y += Projectile.DirectionTo(owner.Center).Y * 2;
            ...
            {
                hits++;
                modifiers.FinalDamage.Flat /= hits;
                if (ret)
                {
                    for (int i = 0; i < 10; i++)
                    {
