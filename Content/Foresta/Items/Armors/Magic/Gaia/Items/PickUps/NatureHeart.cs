...
namespace Crystals.Content.Foresta.Items.Armors.Magic.Gaia.Items.PickUps
{
    public class NatureHeart : ModItem
    {
        ...
        public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float scale,
            int whoAmI)
        {
            Lighting.AddLight(Item.Center , TorchID.Red);
            if (Main.rand.NextFloat() < 0.03f)
            {
                Dust dust;
                Vector2 position = Item.position;
                dust = Main.dust[Terraria.Dust.NewDust(position, Item.width, Item.height, DustID.RedTorch, Main._rand.NextFloat(-2, 2), Main._rand.NextFloat(-2, 2), 0, new Color(255,255,255), 1f)];
            }
        }
